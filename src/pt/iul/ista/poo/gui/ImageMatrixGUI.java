...
public class ImageMatrixGUI extends Observable {
	...
	private void init() {
		...
		frame.setLocation(
				(int)(Toolkit.getDefaultToolkit().getScreenSize().getWidth()/2 - (N_SQUARES_WIDTH * SQUARE_SIZE)/2), 
				(int)(Toolkit.getDefaultToolkit().getScreenSize().getHeight()/2 - (N_SQUARES_HEIGHT * SQUARE_SIZE)/2)
				);
		initImages();
		...
		frame.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				// if (keysUsed.contains(e.getKeyCode())) {
				...
				//System.out.println("Thread " + Thread.currentThread() + " free");
			}
		});
	}
	...
	synchronized void waitForKey() throws InterruptedException {
		...
		//System.out.println("Thread " + Thread.currentThread() + " signaling new key " + lastKeyPressed);
		setChanged();
...
//		frame.repaint();
	...
	// /**
	// *
	// * Remove a given list of images from the main window.
	// *
	// * @param imagesToRemove
	// */
	//
	// public void removeImages(final List<ImageTile> imagesToRemove) {
	...
	private class InfoWindow extends JPanel {
		...
		public void paintComponent(Graphics g) {
			for (ImageTile i : statusImages)
				g.drawImage(imageDB.get(i.getName()).getImage(), i.getPosition().getX() * SQUARE_SIZE, 0, SQUARE_SIZE,
						SQUARE_SIZE, frame);
